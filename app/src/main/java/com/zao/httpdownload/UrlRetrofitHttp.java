...
public class UrlRetrofitHttp {
    ...
    private static UrlRetrofitHttp sIsntance;

    public static UrlRetrofitHttp getInstance() {
        if (sIsntance == null) {
            synchronized (UrlRetrofitHttp.class) {
                if (sIsntance == null) {
                    sIsntance = new UrlRetrofitHttp();
                }
            }
        }
        return sIsntance;
    ...
    public void downloadFile(final long range, final String url, final String fileName, final DownloadCallBack downloadCallback) {
        //断点续传时请求的总长度
        File file = new File(Constant.APP_ROOT_PATH_PRE + Constant.ZOU_DIR, fileName);
        String totalLength = "-";
        if (file.exists()) {
                ...
                .subscribe(new Observer<ResponseBody>() {
                    ...
                    public void onNext(ResponseBody responseBody) {
                        ...
                        try {
                            ...
                            String filePath = Constant.APP_ROOT_PATH_PRE + Constant.ZOU_DIR;
